/**
 * Comprehensive Catalog of roadmap.sh formatted Developer Roadmaps
 * Contains complete stages, interconnected topics, prerequisites,
 * difficulty levels, recommendation badges, and resource queries.
 */
...
export function getCatalogRoadmap(slugOrQuery) {
    if (!slugOrQuery) return ROADMAP_CATALOG['frontend'];
    const q = slugOrQuery.toLowerCase().trim();

    if (q.includes('front') || q.includes('html') || q.includes('css') || q.includes('js')) return ROADMAP_CATALOG['frontend'];
    if (q.includes('back') || q.includes('api') || q.includes('server') || q.includes('node') || q.includes('express')) return ROADMAP_CATALOG['backend'];
    if (q.includes('machine') || q.includes('ai') || q.includes('deep') || q.includes('ml') || q.includes('pytorch')) return ROADMAP_CATALOG['machine-learning'];
    if (q.includes('python')) return ROADMAP_CATALOG['python'];
    if (q.includes('full') || q.includes('stack') || q.includes('web')) return ROADMAP_CATALOG['fullstack'];
    if (q.includes('devops') || q.includes('cloud') || q.includes('k8s') || q.includes('kubernetes') || q.includes('docker')) return ROADMAP_CATALOG['devops'];
